...
# ================= GLOBAL SAVE LIMIT LOCK =================
# Satu lock global untuk cleanup, mencegah race condition antar worker
_cleanup_lock = Lock()
...
def cleanup_old_face_files():
    """
    Hapus pasangan file face+frame terlama dari FACE_DIR & BG_DIR
    sampai total file face di bawah FACE_SAVE_LIMIT.
    Bersifat global — semua channel digabung dalam satu limit.
    """
    existing = sorted([
...
def face_worker(worker_id: int = 1):
    ...
    while True:
        ...
        try:
            ...
            # Batasi maks 5 wajah per frame
            faces = faces[:5]

            # Snapshot DB sekali, hindari lock lama
            with _db_lock:
                db_snapshot = load_db().copy()
            ...
            for face in faces:
                ...
                # ===== GLOBAL SAVE LIMIT =====
                # Cleanup dijalankan sebelum menyimpan file baru,
                # dilindungi lock global agar tidak ada race condition
                # antar worker dari channel manapun.
                ...
                try:
                    ...
                    for name, data in db_snapshot.items():
                ...
                if WEBHOOK_URL:
                    try:
                        ...
                        logger.warning(f"[FACE] Webhook queue full")
